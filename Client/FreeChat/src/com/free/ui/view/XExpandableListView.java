...
public class XExpandableListView extends ExpandableListView implements
		OnScrollListener {
	// modified by lixinyu at 2013-12-10, avoid header and footer change state
	// at the same time
	private final int SCROLL_NONE = 0, SCROLL_HEADER = 1, SCROLL_FOOTER = 2;
	private int scrollState = SCROLL_NONE;
	private final int TIP_TIME = 1000;
	...
	// modified by lixinyu at 2013-12-09, automatically add time when refresh
	// finished
	private SimpleDateFormat sdf = new SimpleDateFormat("MM月dd日，HH:mm:ss");
	private Handler handler = new Handler();
	private boolean tipping = false;// 刷新完成后是否正处于提示状态，如果是的话当用户再次刷新时要取消回调
	...
	/**
	 * modified by lixinyu, reload stopRefresh method, show some tip to user
	 * tell them whether has success or not
	 * 
	 * @param success
	 */
	public void stopRefresh(boolean success) {
		if (mPullRefreshing == true) {
			...
			tipping = true;
			handler.postDelayed(r, TIP_TIME);
		}
	}
	...
	public void setRefreshTime(String time) {
		// modified by lixinyu at 2013-12-12, last time TextView is gone at
		// first time
		mHeaderTimeView.setText(mHeaderView.TIP_HEADER_LAST_TIME + time);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			mLastY = ev.getRawY();
			if ((scrollState == SCROLL_NONE || scrollState == SCROLL_HEADER)
					&& getFirstVisiblePosition() == 0
					&& (mHeaderView.getVisiableHeight() > 0 || deltaY > 0)) {
				scrollState = SCROLL_HEADER;
				// the first item is showing, header has shown or pull down.
				updateHeaderHeight(deltaY / OFFSET_RADIO);
				invokeOnScrolling();

			} else if ((scrollState == SCROLL_NONE || scrollState == SCROLL_FOOTER)
					&& getLastVisiblePosition() == mTotalItemCount - 1
					&& (mFooterView.getBottomMargin() > 0 || deltaY < 0)) {
				scrollState = SCROLL_FOOTER;
